...
## Cropping.System
levels(dat_abun$Cropping.System)
taxlev <- c("Cereal", "Mixed", "Perrenial") ##categories represented by at least 3 studies
mod <- rma.mv(hedges, variance, mods= ~0+Cropping.System, 
              random = list(~ 1 | Obs.ID, ~ 1 | StudyNum), tdist=TRUE, data=dat_abun[which(dat_abun$Cropping.System %in% c("Cereal", "Mixed", "Perrenial")),])
...
## Cropping.System
levels(dat_dive$Cropping.System)
taxlev <- c("Cereal", "Mixed", "Pasture", "Perrenial") ##categories represented by at least 3 studies
mod <- rma.mv(hedges, variance, mods= ~0+Cropping.System, 
              random = list(~ 1 | Obs.ID, ~ 1 | StudyNum), tdist=TRUE, 
              data=dat_dive[which(dat_dive$Cropping.System %in% c("Cereal", "Mixed", "Pasture", "Perrenial")),])
