...
my_map_abun <- ggplot() +
  ...
  geom_sf(data = world, fill="transparent", alpha=0.2, colour = "gray75") +
  geom_point(data = locdat_abun, aes(x = long, y = lat, fill = as.factor(nstudy), size=nstudy),  shape = 21, alpha=0.5) +
  theme(legend.position = c(0.8, 0.2)) +
  scale_fill_manual(values = c("brown2", "darkorchid2", "turquoise3")) +
  labs(fill="Studies") + 
...
my_map_dive <- ggplot() +
  geom_sf(data = world, inherit.aes = F) +
  geom_sf(data = world, fill="transparent", alpha=0.2, colour = "gray75") +
  geom_point(data = locdat_dive, aes(x = long, y = lat, fill = as.factor(nstudy), size=nstudy), shape = 21, alpha=0.5) +
  theme(legend.position = c(0.8, 0.2)) +
  scale_fill_manual(values = c("brown2", "darkorchid2", "turquoise3")) +
  labs(fill="Studies") + 
  labs(x="Longitude", y="Latitude") +
